...
import {
  ...
  set_ligands_radius,
} from "@/store/slices/ui_reducer";
import {
  capitalize_only_first_letter_w,
  yield_nomenclature_map_profile,
} from "@/my_utils";
import {
  IconVisibilityOn,
  IconToggleSpin,
...
function LigandTaxonomyDropdown(props: {
  count: number;
  species: LigandAssociatedTaxa;
}) {
  return (
    <DropdownMenu>
...
function LigandStructuresDropdown(props: {
  count: number;
  structures: LigandAssociatedStructure[];
  info: LigandInfo;
}) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="max-h-80 overflow-y-scroll">
        {props.structures
          .toSorted((s1, s2) =>
            Number(s1.src_organism_names[0] > s2.src_organism_names[0])
          )
          .map((struct, i) => (
                  ...
                  <div className="italic text-white flex gap-2 flex-row">
                    ...
                    <TaxonomyDot
                      className={`w-2 h-2 ${(() => {
                        ...
                        else if (struct.superkingdom == 2157)
                          return "fill-orange-500";
                        else if (struct.superkingdom === 2759)
...
const chemical_structure_link = (ligand_id: string | undefined) => {
  ...
  return `https://cdn.rcsb.org/images/ccd/labeled/${
    ligand_id.toUpperCase()[0]
  }/${ligand_id.toUpperCase()}.svg`;
};
...
const lig_data_to_tree = (lig_data: LigandInstances) => {
  return lig_data.map(([lig, structs]) => ({
    ...
    search_aggregator: (
      ...
      structs.reduce(
        (acc: string, next) =>
          acc + next.rcsb_id + next.tax_node.scientific_name,
        ""
      )
    ).toLowerCase(),
...
export default function Ligands() {
  ...
  const [refetchParentStruct] =
    ribxz_api.endpoints.routersRouterStructStructureProfile.useLazyQuery();
  ...
  const current_ligand = useAppSelector(
    (state) => state.ui.ligands_page.current_ligand
  );

  const [surroundingResidues, setSurroundingResidues] = useState<ResidueList>(
    []
  );
  const [parentStructProfile, setParentStructProfile] =
    useState<RibosomeStructure>({} as RibosomeStructure);
  ...
  useEffect(() => {
    ...
    var nom_map: any = {};
    for (let polymer of [
      ...parentStructProfile.rnas,
      ...parentStructProfile.proteins,
    ]) {
  ...
  //     }
  // );
  const current_selected_target = useAppSelector(
    (state) => state.homepage_overview.selected
  );
  useEffect(() => {
                ...
                <div className="p-4 space-y-2">
                  <div className="flex flex-row space-x-4">
                    <TreeSelect
                      ...
                      filterTreeNode={(input, treenode) => {
                        return (treenode.search_aggregator as string).includes(
                          input.toLowerCase()
                        );
                      }}
                      onChange={(value: string, _) => {
                        var [chemId, rcsb_id_selected] = value.split("_");
                        const lig_and_its_structs = lig_state.data.filter(
                          (kvp) => {
                            var [lig, structs] = kvp;
                  ...
                  <div className="flex flex-row w-full justify-between">
                    <Button
                      onClick={() => {
                        ...
                        ctx
                          .create_ligand_and_surroundings(
                            current_ligand?.ligand.chemicalId,
                            lig_state.radius
                          )
                          .then((ctx) =>
                  ...
                  <ScrollArea className="h-[90vh] overflow-scroll  no-scrollbar space-y-4 mt-16">
                    ...
                    >
                      <AccordionItem value="item">
                        <AccordionTrigger className="text-xs rounded-sm hover:cursor-pointer hover:bg-muted border p-1">
                          {lig_state.current_ligand?.ligand.chemicalId} Chemical
                          Structure
                        </AccordionTrigger>
                        {current_ligand ? (
                    ...
                    >
                      <AccordionItem value="item-1">
                        ...
                        <AccordionContent>
                          <p className="text-xs">
                            {
                              lig_state.current_ligand?.ligand
                          ...
                          <div className="flex flex-row justify-between w-full pr-4">
                            <span>
                              ...
                              {lig_state.current_ligand === null ? (
                                ...
                                lig_state.current_ligand?.ligand.chemicalId +
                                " in " +
                                lig_state.current_ligand?.parent_structure
                                  .rcsb_id
                              )}
                            </span>
                            <span className="font-light">
                              Binding Pocket Details
                            </span>
                          </div>
                        </AccordionTrigger>
                        <AccordionContent>
                    ...
                    <Accordion
                      ...
                      onValueChange={(value) =>
                        setIsAccordionOpen(value === "prediction")
                      }
                      className="border p-1 rounded-md"
                    >
                          ...
                          <div className="flex items-center space-x-2 text-xs p-1 border-b mb-2">
                              ...
                              onClick={() => {
                                ...
                                dispatch(
                                  fetchPredictionData({
                                    ...
                                    src: current_ligand?.parent_structure
                                      .rcsb_id,
                            ...
                            >
                              ...
                              {ligands_state.prediction_pending ? (
                                <>
                                  <Spinner />{" "}
                                  <span className="mx-2">Calculating</span>
                                </>
                              ) : (
                                "Render Prediction"
                              )}
                                    ...
                                    <AccordionItem value={"other"}>
                                      <AccordionTrigger>
                                        <div
                                          className="flex flex-row justify-start w-64 border-dashed border p-1 rounded-md border-black hover:bg-pink-300"
                                          onMouseEnter={() =>
                                            ctx_secondary?.highlightChain(
                                              chain.target.auth_asym_id
                                            )
                                          }
                                          onMouseLeave={() =>
                                            ctx_secondary?.removeHighlight()
                                          }
                                          onClick={(e) => {
                                            e.stopPropagation();
