...
const sort_by_polymer_class = (a: Polymer, b: Polymer): number => {
  ...
  const [, prefixA, numberA] =
    a.nomenclature[0].match(/([a-zA-Z]+)(\d*)/) || [];
  const [, prefixB, numberB] =
    b.nomenclature[0].match(/([a-zA-Z]+)(\d*)/) || [];
    ...
    >
      <div className="border border-gray-200 rounded-md shadow-inner bg-slate-100 p-2">
        ...
        <div className="space-y-0">
          ...
          <InfoRow
            title="Deposition Year"
            value={parseDateString(data.deposition_date).year}
          />
          <InfoRow
          ...
          <InfoRow
            ...
            value={data?.src_organism_names.join(", ")}
          />
          {data?.host_organism_names?.length > 0 && (
            <InfoRow
              title="Host Organism"
              value={data.host_organism_names[0]}
            />
          )}
        </div>
      </div>
...
const availableLandmarks = {
  PTC: {
    ...
    longDescription:
      "The PTC is the active site of the ribosome where peptide bond formation occurs.",
  },
  NPET: {
    title: "NPET",
    description: "Nascent Peptide Exit Tunnel",
    longDescription:
      "The NPET is a channel through which newly synthesized proteins exit the ribosome.",
  },
};
...
const TunnelLandmarkComponent: React.FC<{
  ...
  const tunnel_loci = useAppSelector(
    (state) => state.structure_page.tunnel.loci
  );
  ...
  return (
    <LandmarkItem
      {...availableLandmarks["NPET"]}
      rcsb_id={rcsb_id}
      landmark_actions={defaultTunnelActions}
    />
  );
};

const StructureEasyAccessPanel = ({
  data,
  isLoading,
}: {
  data: RibosomeStructure;
  ...
  const ctx = useContext(MolstarContext);
  const dispatch = useAppDispatch();
  const selected_polymers = useAppSelector(
    (state) => state.structure_page.selected
  );
  const toggleComponent = (id: string) => {
    dispatch(set_id_to_selection(id));
  };
  const [newBookmarkName, promptForNewBookmark] = useUserInputPrompt(
    "Enter a name for the new bookmark:"
  );
  const [bindingSiteName, promptForBindingSiteName] = useUserInputPrompt(
    "Enter a name for the binding site object:"
  );
    ...
    <div className="space-y-4 shadow-inner bg-slate-100 p-2 border-gray-200 rounded-md">
            ...
            <MolecularComponentBadge
              ...
              onClick={() => {
                ...
                ctx?.select_chain(
                  ...
                  selected_polymers.includes(component.auth_asym_id)
                    ? "remove"
                    : "add"
                );
              }}
              onMouseEnter={
                ctx
                  ? () => ctx.highlightChain(component.auth_asym_id)
                  : undefined
              }
      ...
      <div>
        <Button
          onClick={() => {
            (async () => {
              await ctx?.create_neighborhood_selection_from_expr(
                ctx?.select_expression("j")
              );
            })();
          }}
        ></Button>
...
export default function StructurePage({
  ...
  const selections = useAppSelector(
    (state) => state.structure_page.saved_selections
  );
  useEffect(() => {
