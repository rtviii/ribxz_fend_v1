...
import {
  StateObjectRef,
  StateObjectSelector,
} from "molstar/lib/mol-state/object";
...
export class MolstarRibxz {
  ...
  select_residueCluster = (
    ...
    const data: any =
      this.ctx.managers.structure.hierarchy.current.structures[0]?.cell.obj
        ?.data;
  ...
  select_chain = (
    auth_asym_id: string,
    modifier: "set" | "add" | "remove" | "intersect"
  ) => {
    const data =
      this.ctx.managers.structure.hierarchy.current.structures[0]?.cell.obj
        ?.data;
  ...
  create_neighborhood_selection_from_expr = async (e: Expression) => {
    const RADIUS = 5;
    let structures =
      this.ctx.managers.structure.hierarchy.current.structures.map(
        (structureRef, i) => ({ structureRef, number: i + 1 })
      );
    ...
    surroundingsSel.apply(
      ...
      createStructureRepresentationParams(
        this.ctx,
        struct.structureRef.cell.obj?.data,
        { type: "ball-and-stick" }
      ),
    ...
    const selection = compile<StructureSelection>(e)(
  ...
  async upload_mmcif_chain({
    ...
    const trajectory = await this.ctx.builders.structure.parseTrajectory(
      data,
      "mmcif"
    );
  ...
  async create_ball_and_stick_representation(
    selection: StateObjectSelector,
    label: string
  ) {
    const update = this.ctx.build();
  ...
  async toggle_visibility_by_ref(representation: any, on_off: boolean) {
    ...
    setSubtreeVisibility(
      this.ctx.state.data,
      representation["components"]["polymer"]["ref"],
      on_off
    );
  }
  async color_all_white() {
    const componentGroups =
      this.ctx.managers.structure.hierarchy.currentComponentGroups;
  ...
  select_multiple_residues(
    chain_residues_tuples: [string, number[]][],
    structure?: Structure
  ) {
    const groups: Expression[] = [];
  ...
  select_multiple_polymers = async (
  ...
  create_multiple_polymers = async (
    auth_asym_ids: string[],
    object_name: string
  ) => {
    let structures =
  ...
  async select_focus_ligand(
    ...
    const ligand_selection = compiled(
      new QueryContext(struct.structureRef.cell.obj?.data!)
    );
  ...
  async get_selection_constituents(
    ...
    surroundingsSel.apply(
      ...
      createStructureRepresentationParams(
        this.ctx,
        struct.structureRef.cell.obj?.data,
        { type: "label", typeParams: { level: "residue" } }
      ),
  ...
  async create_ligand_and_surroundings(
    chemicalId: string | undefined,
    radius: number
  ) {
    if (!chemicalId) {
  ...
  async select_focus_ligand_surroundings(
    ...
    const surr_selection = compile<StructureSelection>(
      surroundingsWithoutLigand
    )(new QueryContext(struct.structureRef.cell.obj?.data!));
  ...
  dynamicSuperimpose(pivot_auth_asym_id: string) {
    ...
    const siteVisual = async (
      s: StateObjectRef<PSO.Molecule.Structure>,
      pivot: Expression
    ) => {
      const center =
        await this.ctx.builders.structure.tryCreateComponentFromExpression(
          s,
          pivot,
          "pivot"
        );
    ...
    return this.ctx.dataTransaction(async () => {
      ...
      const structureRefs =
        this.ctx.managers.structure.hierarchy.current.structures;
      const selections = structureRefs.map((s) =>
        StructureSelection.toLociWithCurrentUnits(
          query(new QueryContext(s.cell.obj!.data))
        )
      );
