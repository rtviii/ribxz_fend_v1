...
import { MolstarRibxz, ResidueList, StateElements } from "../molstar_ribxz";
...
export class MolstarDemoBsites extends MolstarRibxz {
  //@ts-ignore
  ctx: PluginUIContext;
  constructor() {
    super();
  ...
  async init(parent: HTMLElement, spec: PluginUISpec) {
    ...
    this.ctx.representation.structure.themes.colorThemeRegistry.add(
      VaryingResidueColorThemeProvider
    );
  ...
  applyBsiteColors = async (bsite: ScoredBsite) => {
    this.ctx.dataTransaction(async () => {
      for (const s of this.ctx.managers.structure.hierarchy.current
        .structures) {
        ...
        const repr =
          this.ctx.managers.structure.hierarchy.current.structures[0]
            .components[0].representations[0].cell;
