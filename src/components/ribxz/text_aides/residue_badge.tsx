...
const AminoAcidColorTable: Record<
  string,
  { color_name: string; rgb: number[]; hex: string }
> = {
...
const NucleotidesColorTable: Record<
  string,
  { color_name: string; rgb: number[]; hex: string }
> = {
...
export const ResidueBadge = ({
  ...
  const residue_color_border = () => {
    if (residue.label_comp_id === undefined || residue.label_comp_id === null) {
      ...
      Object.keys(NucleotidesColorTable).includes(residue.label_comp_id)
    ) {
      return [NucleotidesColorTable[residue.label_comp_id].hex, "border"];
    } else if (
      Object.keys(AminoAcidColorTable).includes(residue.label_comp_id)
    ) {
  ...
  return (
    ...
    >
      <Badge
        variant="outline"
        className={`hover:bg-muted hover:cursor-pointer  ${border}  border-2`}
      >
        <div className="flex flex-row justify-between w-fit ">
          <span
            className="text-xs font-bold w-fit px-1 text-center"
            style={{ color: color }}
          >
            {residue.label_comp_id}
          </span>
          <span className="text-xs font-light w-fit px-1 text-center text-black">
            {residue.auth_seq_id}
          </span>
        ...
        {show_parent_chain ? (
          <div className="flex flex-row justify-between w-fit border-l-2 ">
            <span className="text-xs w-fit font-medium px-1 text-black">
              {residue.polymer_class}
            </span>
            <span className="text-xs w-fit font-light px-1 text-black">
